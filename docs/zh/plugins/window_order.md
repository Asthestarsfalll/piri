...
Window Order 插件可以根据配置的优先级自动重排工作区中的窗口顺序。窗口会按照配置的权重值进行排序，权重值越大，窗口越靠左。
...
- **`enable_event_listener`**: 是否启用事件监听。启用后，当窗口布局改变或新窗口打开时会自动重排窗口（仅在配置的 `workspaces` 中生效）
...
Window Order 插件使用智能算法来最小化窗口移动次数：

1. **获取当前状态**: 获取当前工作区中所有平铺窗口的列位置
2. **计算目标位置**: 根据配置的权重值计算每个窗口的目标位置
3. **优化移动序列**: 使用贪心算法找到移动次数最少的方案
4. **执行移动**: 依次移动窗口到目标位置
...
- **保持相同权重窗口的相对顺序**: 相同权重的窗口保持当前相对顺序，避免不必要的移动

### 移动策略

当移动窗口从列 A 到列 B 时：
- 如果 A < B（向右移动）：列 (A, B] 的窗口左移 1 位
- 如果 A > B（向左移动）：列 [B, A) 的窗口右移 1 位

算法会模拟每次移动后的状态，选择最优的移动序列。
...
- **工作区组织**: 将常用应用（如浏览器、编辑器）固定在工作区左侧
- **多窗口管理**: 在多个相同应用窗口时，保持它们的相对顺序
- **自动化布局**: 通过事件监听自动维护窗口顺序
